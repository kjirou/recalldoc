...
import { deleteFootprint } from "./reducers";
import { Storage, saveConfig, saveFootprints } from "./storage";
import { Config, Footprint, rotateIndex, searchFootprints } from "./utils";
...
export const useVariables = (
  ...
  const searchedFootprints = useMemo(
    () => searchFootprints(footprints, inputValue, config.enableRomajiSearch),
    [config, footprints, inputValue]
  );
  const displayableFootprints = searchedFootprints.slice(0, 10);
  const cursoredFootprint =
    displayableFootprints[
      rotateIndex(displayableFootprints.length, cursoredIndex)
    ];

  const onInput = useCallback<SearcherProps["onInput"]>(
  ...
  const onKeyDown = useCallback<SearcherProps["onKeyDown"]>(
    (key, isComposing, stopPropagation, preventDefault) => {
      stopPropagation();
  ...
  const onClickDeleteButton = useCallback(
    (url: SearcherFootprintProps["url"]) => {
      ...
      if (deleted) {
        ...
        throw new Error(
          "The deleted footprint must exist in searched footprints."
        );
      }
    },
    [displayableFootprints]
  );
  const onChangeCheckboxOfRomajiSearch = useCallback<
    SearcherProps["onChangeCheckboxOfRomajiSearch"]
  >((checked) => {
    setConfig((s) => ({
      ...s,
  ...
  const onChangeSelectOfStartupKeyCombination = useCallback<
    SearcherProps["onChangeSelectOfStartupKeyCombination"]
...
export const useStorageSynchronization = (
  storage: Storage,
  config: Config,
  footprints: Footprint[]
): void => {
  ...
  useEffect(() => {
    if (
      config !== previousConfigRef.current ||
      footprints !== previousFootprintsRef.current
    ) {
      // TODO: 処理順序保証、二重実行回避。
      // TODO: ummount時のキャンセル。
...
export const SearcherContainer: FC<Props> = (props) => {
  const { config, footprints, searcherProps } = useVariables(
    props.config,
    props.footprints,
    props.onClose
  );
  ...
  const portalRoot = usePortalRoot(
    props.portalDestination,
    props.enableShadowDom
  );
  ...
  return portalRoot
    ? createPortal(<Searcher {...searcherProps} />, portalRoot as any)
    : null;
};
