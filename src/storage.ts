import { updateFootprint as updateFootprintReducer } from "./reducers";
import {
  Config,
  Footprint,
  OldFootprint,
...
export const loadFootprints = async (
  storage: Storage
): Promise<Footprint[]> => {
...
export const loadFootprintLikes = async (
  storage: Storage
): Promise<(Footprint | OldFootprint)[]> => {
  const rawFootprints = await storage.loadItem(storage.footprintsKey);
  return rawFootprints ? JSON.parse(rawFootprints) : [];
};
...
export const updateFootprintOfEsaCategory = (
  storage: Storage,
  origin: string,
  hash: string
): Promise<void> => {
...
export const updateFootprintOfKibelaFolder = (
  storage: Storage,
  url: string
): Promise<void> => {
  const urlObj = new URL(url);
  const folderPath = decodeURIComponent(
    urlObj.pathname.replace(/^\/notes\/folder\//, "")
  );
