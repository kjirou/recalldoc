...
export const isStartupKeyCombinationType = (
  value: any
): value is Config["startupKeyCombination"] =>
...
export const splitSearchQueryIntoMultipulKeywords = (
  query: string
): string[] => {
...
export const searchFootprints = (
  footprints: Footprint[],
  searchQuery: string,
  enableRomajiSearch: boolean
): Footprint[] => {
  const keywordMatchers = splitSearchQueryIntoMultipulKeywords(searchQuery).map(
    (keyword) => {
 ...
 */
export const adjustOldFootprints = (
  footprintLikes: (Footprint | OldFootprint)[]
): Footprint[] => {
  const isOldFootprint = (arg: any): arg is OldFootprint =>
    arg.directories === undefined && arg.title !== undefined;
