...
import { Config, Footprint, createDefaultConfig } from "./utils";
import { Storage } from "./storage";

describe("useVariables", () => {
  const renderUseVariables = (...args: Parameters<typeof useVariables>) => {
    return renderHook((args) => useVariables(...args), { initialProps: args });
  };
  ...
  describe("searcherProps", () => {
    ...
    describe("startupKeyCombination, onChangeSelectOfStartupKeyCombination", () => {
      test("it works", () => {
        ...
        act(() => {
          result.current.searcherProps.onChangeSelectOfStartupKeyCombination(
            "2"
          );
        });
...
describe("useStorageSynchronization", () => {
  const renderUseStorageSynchronization = (
    ...args: Parameters<typeof useStorageSynchronization>
  ) => {
    return renderHook((args) => useStorageSynchronization(...args), {
      initialProps: args,
    });
  };
  let storage: Storage;
  ...
  test("it does not call save methods when all args are the same", () => {
    ...
    const { rerender } = renderUseStorageSynchronization(
      storage,
      config,
      footprints
    );
...
describe("usePortalRoot", () => {
  const renderUsePortalRoot = (...args: Parameters<typeof usePortalRoot>) => {
    return renderHook((args) => usePortalRoot(...args), { initialProps: args });
  };
...
describe("SearcherContainer", () => {
  ...
  test("it calls props.onClose when the user clicks the backdrop", async () => {
    const { getByTestId } = render(<SearcherContainer {...defaultProps} />);
