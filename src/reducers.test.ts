import { updateFootprint } from "./reducers";
...
describe("updateFootprint", () => {
  const table: {
  ...
  }[] = [
    {
      ...
      footprints: [{ directories: ["foo"], url: "https://example.com/foo" }],
    ...
    },
    {
      ...
      expected: [{ directories: ["foo2"], url: "https://example.com/foo" }],
    },
  ];
  ...
  test("it returns up to 1000", () => {
    ...
    expect(
      updateFootprint({ directories: [], url: "https://not-example.com" })(
        footprints1000
      )
    ).toHaveLength(1000);
  });
